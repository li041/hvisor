...
static VIRTUAL_IPI_TRACE_SEQ: AtomicU64 = AtomicU64::new(0);
const VIRTUAL_IPI_TRACE_LIMIT: u64 = 64;
...
fn trace_virtual_ipi_transition(
    operation: &str,
    target_cpu: usize,
    bits: u32,
    old: u32,
    ...
    let seq = VIRTUAL_IPI_TRACE_SEQ.fetch_add(1, Ordering::Relaxed);
    if seq < VIRTUAL_IPI_TRACE_LIMIT {
        info!(
            "[HVDBG:guest-ipi] seq={} op={} src_cpu={} dst_cpu={} bits={:#x} old={:#x} new={:#x} blocking={}",
            seq,
            operation,
            this_cpu_id(),
            target_cpu,
...
fn virtual_ipi_send(target_guest_cpu: usize, ipi_bits: u32, blocking: bool) -> HvResult {
    ...
    let new = old | pending;
    if old != new {
        trace_virtual_ipi_transition("set", target_cpu, pending, old, new, blocking);
    }
    if old == 0 && new != 0 {
    ...
    match reg {
        ...
        IPI_CLEAR => {
            if mmio.is_write {
                ...
                let new = old & !mask;
                if old != new {
                    trace_virtual_ipi_transition("clear", cpu, mask, old, new, false);
                }
                if old != 0 && new == 0 {
